...
const escapeValue = (value: string, characterSet: string[]): string =>
	value
		...
		.map((char: string): string =>
			characterSet.includes(char) ? char : characterSet[0]!
		)
		.join('');
...
export default function SplitFlapDisplay({
	...
	const updateValue = useCallback(() => {
		...
		if (
			updateTimer.current ||
			shadowPrevValue.current === escapedFinalValue
		) {
			return;
		}
		...
		const nextValue = finalChars
			.map((char: string, idx: number): string => {
				...
				const nextChar =
					currChar === char ||
					(charIdx === 0 && !characterSet.includes(char))
						? currChar
	...
	if (padDirection === 'right') {
		prevChars = [
			...prevValue.split(''),
			...getMinLengthFill(prevValue, characterSet, minLength),
		];
		currChars = [
			...currValue.split(''),
			...getMinLengthFill(currValue, characterSet, minLength),
		];
	} else {
		prevChars = [
			...getMinLengthFill(prevValue, characterSet, minLength),
